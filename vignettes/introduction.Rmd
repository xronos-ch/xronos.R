...
The command `chron_data()` Will probably be the most common entry point for most of the users. For advanced users, there is a different version of the request using the command `xronos_query` that uses two lists, one for the filters, the other one for the values that specify the filters. In that sense, the same result as above could be achieved using the following command:

```{r}
xronos_query(c("country", "material"), list("Switzerland", c("bone", "charcoal")))
```
